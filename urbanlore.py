...
def generate_corpus(target_words, output_dir):
...
def extract_facts(corpus_file, output_dir):
...
def generate_qa(facts_file, corpus_file, output_dir, num_qa, num_instructions):
...
def run_all(target_words):
    ...
    ctx.invoke(generate_corpus, target_words=target_words, output_dir="corpus")
    ctx.invoke(extract_facts, corpus_file="corpus/city_corpus.txt", output_dir="corpus")
    ctx.invoke(generate_qa, facts_file="corpus/facts.json", corpus_file="corpus/city_corpus.txt", 
               output_dir="dataset", num_qa=1000, num_instructions=500)
